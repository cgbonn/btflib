...
function obj = create_ubo_bdi(obj, meta, data)
    ...
    p.addRequired('chunks', @(x) isnumeric(x.chunks) && ...
        (ismatrix(x.chunks) && all(size(x.chunks) == [nC * nL * nV, w * h]) || ...
        ismatrix(x.chunks) && all(size(x.chunks) == [obj.meta.chunk_size, obj.meta.num_chunks]) || ...
        ndims(x.chunks) == 5 && all(size(x.chunks) == [nC, nL, nV, w, h]) || ...
        ndims(x.chunks) == 3 && all(size(x.chunks) == [nC, nL, nV])));
    p.parse(data);
    
    obj.data.chunks = p.Results.chunks.chunks;
    clear p;
