...
% *************************************************************************
%
% Decode all eigen-textures from a full matrix factorized (FMF) BTF file in
...
function textures = get_fmf_eigen_textures(obj)
    ...
    if size(U, 1) == nC * nL * nV
        ...
        textures = permute(reshape(SxV, 1, w, h, []), [3, 2, 1, 4]);
    elseif size(U, 1) == nC * h * w
        % images stacked column-wise
        textures = permute(reshape(U, nC, w, h, []), [3, 2, 1, 4]);
    else
