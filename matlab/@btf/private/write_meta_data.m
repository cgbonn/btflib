...
function write_meta_data(fid, meta)
    ...
    size_bytes = utils.sizeof('uint32') + utils.sizeof(struct_version) + ...
        numel(measurement_setup) + ...
        numel(image_sensor) + ...
        numel(light_source) + ...
        utils.sizeof(single(meta.ppmm)) + ...
        utils.sizeof(single(meta.rgb_scale_factor));
    
    if struct_version > 1
        size_bytes = size_bytes + utils.sizeof(uint32(meta.cosine_flag));
    end
